...
def save_as_pickle(filename, data, iter):
...
def MCTS_self_play(chessnet,num_games,cpu):
    for idxx in range(0,num_games):
        ...
        del dataset
        save_as_pickle("dataset_cpu%i_%i_%s" % (cpu,idxx, datetime.datetime.today().strftime("%Y-%m-%d")),dataset_p,'mcts')
...
if __name__=="__main__":
    ...
    net_to_play="current_net_trained_iter2.pth.tar"
    ...
    processes = []
    for i in range(6):
        p = mp.Process(target=MCTS_self_play,args=(net,35,i))
        p.start()
        processes.append(p)
    for p in processes:
        p.join()

...
class ConvBlock(nn.Module):
    def __init__(self):
        ...
        self.dropout = nn.Dropout(p=0.03)  # Dropout with 3% probability
...
def train(net, dataset, epoch_start=0, epoch_stop=100, cpu=0, validation_split=0.2, patience=10):
    ...
    train_set = board_data([dataset[i] for i in train_indices])
    val_set = board_data([dataset[i] for i in val_indices])
    
    train_loader = DataLoader(train_set, batch_size=30, shuffle=True, num_workers=0, pin_memory=False)
    val_loader = DataLoader(val_set, batch_size=30, shuffle=False, num_workers=0, pin_memory=False)
    ...
    ax.scatter([e for e in range(1, epoch + 2)], losses_per_epoch, label='Training Loss')
    ax.scatter([e for e in range(1, epoch + 2)], val_losses_per_epoch, label='Validation Loss')
    ax.set_xlabel("Epoch")
    ...
    print(os.path.join("./model_data/", "Loss_vs_Epoch_%s.png"))

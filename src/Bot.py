...
def minimax_search(board: Board.GUI_Board, depth = 100, maximizing = False):
    global d
    global best_moves
    global nodes_count
    d = depth
    ...
    #Make the move
    a = Board.get_pos_from_coord(chosen_move[0:2])
    b = Board.get_pos_from_coord(chosen_move[2:4])
    best_piece = board.get_piece_from_pos(a)
    dst_sq = board.get_square_from_pos(b)

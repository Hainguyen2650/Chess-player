...
class arena():
    ...
    def play_round(self):
        ...
        while checkmate == False and current_board.move_count <= 100:
            ...
            if current_board.player == 0:
                best_move, _ = UCT_search(current_board,100,white)
            elif current_board.player == 1:
                best_move, _ = UCT_search(current_board,100,black)
    ...
    def evaluate(self, num_games: int,cpu):
        current_wins = 0
        ...
        a = current_wins
        b = num_games
        print("Current_net wins ratio: %.3f" % a/b)
...
if __name__=="__main__":
    ...
    current_net="current_net_trained_iter3.pth.tar"; best_net="best_net.pth.tar"
    ...
    nums_gem = 1
    for i in range(6):
        p = mp.Process(target=fork_process,args=(arena(current_chessnet,best_chessnet),nums_gem,i))
        p.start()
        processes.append(p)
    for p in processes:
        p.join()
